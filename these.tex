...
\addcontentsline{toc}{section}{Appendix A: Complete Database Schema}
...
\begin{tikzpicture}[
    node distance=2cm,
    entity/.style={rectangle, draw, minimum width=3cm, minimum height=1cm, align=center},
    relationship/.style={diamond, draw, minimum width=2cm, minimum height=1cm, align=center},
    attribute/.style={ellipse, draw, minimum width=2cm, minimum height=0.8cm, align=center}
]

% Entities
\node[entity] (users) {Users};
\node[entity, right=of users] (requests) {Requests};
\node[entity, right=of requests] (metrics) {Metrics};
\node[entity, below=of requests] (transactions) {Transactions};
\node[entity, above=of requests] (models) {Models};
\node[entity, left=of users] (family) {Family};

% Relationships
\draw[thick, ->] (users) -- (requests) node[midway, above] {1:N};
\draw[thick, ->] (requests) -- (metrics) node[midway, above] {1:1};
\draw[thick, ->] (users) -- (transactions) node[midway, right] {1:N};
\draw[thick, ->] (requests) -- (transactions) node[midway, right] {1:N};
\draw[thick, -] (models) -- (requests) node[midway, right] {reference};
...
\newpage
\section*{Appendix B: Implementation Code Samples}
\addcontentsline{toc}{section}{Appendix B: Implementation Code Samples}

This appendix provides core implementation code for key algorithms described in the thesis.

\subsection*{B.1 Provider Filtering Algorithm}
...
\addcontentsline{toc}{section}{Appendix C: Provider and Model Reference Tables}
...
\addcontentsline{toc}{section}{Appendix D: Deployment Architecture}
...
\subsection*{D.2 Environment Variables}

\begin{listing}[H]
\begin{minted}{bash}
# Database
DATABASE_URL=postgresql://user:pass@host:5432/makehub
REDIS_URL=redis://localhost:6379

# Provider API Keys (20+ providers)
API_KEY_OPENAI=sk-...
API_KEY_ANTHROPIC=sk-ant-...
API_KEY_TOGETHER=...
API_KEY_FIREWORKS=...
API_KEY_GROQ=...
# (... 15+ more provider keys)
...
# Monitoring
NTFY_URL=https://ntfy.sh/makehub-alerts
DATADOG_API_KEY=...
...
\subsection*{D.3 Deployment Sequence Diagram}

\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=2cm]
\node (client) [rectangle, draw, minimum width=2cm] {Client};
\node (gateway) [rectangle, draw, minimum width=2cm, below=of client] {API Gateway};
\node (db) [rectangle, draw, minimum width=2cm, below=of gateway] {PostgreSQL};
\node (redis) [rectangle, draw, minimum width=2cm, right=of db] {Redis};
\node (provider) [rectangle, draw, minimum width=2cm, below=of db] {Provider};

\draw[->] (client) -- (gateway) node[midway, right] {HTTP POST};
\draw[->] (gateway) -- (db) node[midway, right] {Query models};
\draw[->] (gateway) -- (redis) node[midway, above] {Check cache};
\draw[->] (gateway) -- (provider) node[midway, right] {Forward request};
\draw[->] (provider) -- (gateway) node[midway, left] {SSE stream};
\draw[->] (gateway) -- (client) node[midway, left] {Proxy stream};
\end{tikzpicture}
\caption{Request flow deployment diagram}
\end{figure}
